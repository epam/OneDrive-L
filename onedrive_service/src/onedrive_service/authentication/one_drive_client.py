...
class OneDriveClient(BaseOneDriveClient):
    ...
    def __init__(self, config: dict) -> None:
        ...
        auth_provider = onedrivesdk.AuthProvider(http_provider, client_id, scopes, session_type=Session)

        super().__init__(API_BASE_URL, auth_provider, http_provider)

    def authenticate(self, client_secret=None, redirect_uri="http://localhost:8080/", auth_code=None):
        """ Authenticate process for OneDrive for personal use.
        """

        client_secret = client_secret if client_secret is not None else self._config["auth"]["client_secret"]
